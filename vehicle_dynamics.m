function dx = vehicle_dynamics(states, U)
    ...
    dx(1,1)=x_dot+(a+(-Fyf*delta)/m+psi_dot*y_dot)*Ts;
    dx(2,1)=y_dot+((Fyf+Fyr)/m-psi_dot*x_dot)*Ts;
    dx(3,1)=psi+psi_dot*Ts;
    dx(4,1)=psi_dot+((Fyf*lf*cos(delta)-Fyr*lr)/Iz)*Ts;
    dx(5,1)=X+(x_dot*cos(psi)-y_dot*sin(psi))*Ts;
    dx(6,1)=Y+(x_dot*sin(psi)+y_dot*cos(psi))*Ts;
